...
import { LayoutGrid, Plus, ArrowLeft, ExternalLink, Trash2, FileText, Code2, Copy, Check, Sparkles, BookOpen, Layers } from 'lucide-react';

export default function WebDevTracker({ tasks = [], onAddTask, onAddApproach, onAddCode, onDeleteTask, onBack, isAdmin = false }) {
  ...
  // 1. Submit New Web Dev Question / Task
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            {onBack && (
              <button
                onClick={onBack}
                className="btn-secondary text-xs py-2 px-3 flex items-center gap-1.5 border border-indigo-500/30 text-indigo-300 hover:text-indigo-200 transition-all cursor-pointer"
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-100">
                Web Development Daily Learning Tracker
              </h2>
              <p className="text-xs text-slate-400">Log daily web development topics, upload architecture approaches, and attach production solution code</p>
            </div>
          </div>
      ...
      <div className="glass-card overflow-hidden space-y-4 p-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
          <div>
            <h3 className="text-lg font-bold text-slate-100">Logged Web Dev Tasks & Projects ({tasks.length})</h3>
            <p className="text-xs text-slate-400">Track questions, concepts, approaches & solution code</p>
          </div>
          ...
          {isAdmin ? (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="btn-primary text-xs py-2.5 px-4 flex items-center gap-1.5 shadow-md shadow-sky-500/20"
              >
                <Plus className="w-4 h-4 text-slate-950" />
                <span>1. + Log Task</span>
              </button>
              ...
              <button
                ...
                className="btn-accent text-xs py-2.5 px-4 flex items-center gap-1.5 shadow-md shadow-indigo-500/20"
              >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/70 text-xs font-extrabold text-slate-200 uppercase tracking-wider">
                <th className="py-4 px-6">Task Title & Topic</th>
                <th className="py-4 px-4">Category</th>
                <th className="py-4 px-4">Date Completed</th>
              ...
              {tasks.length === 0 ? (
                <tr>
                  <td colSpan="5" className="py-12 text-center text-slate-400 font-semibold">
                    No Web Dev tasks logged yet. Click <b>"+ Log Task"</b> to add your first web development solve!
                  </td>
                </tr>
              ) : (
                tasks.map((t) => (
                    ...
                    <td className="py-4 px-6 text-right space-x-2">
                      {t.approach || t.solution_code ? (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 border border-sky-500/30 text-xs font-bold transition-all cursor-pointer"
                        >
                          📖 View Approach & Code
                        </button>
                      ) : (
                        <div className="inline-flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 rounded-lg bg-indigo-500/10 text-indigo-300 hover:bg-indigo-500/20 border border-indigo-500/30 text-xs font-bold transition-all cursor-pointer"
                          >
                            + Code
                          </button>
                        </div>
      ...
      {/* ---------------- MODAL 1: Log Web Dev Task / Question ---------------- */}
      {showLogModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card max-w-md w-full p-6 space-y-4 border border-sky-500/40 animate-scaleUp">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <Plus className="w-5 h-5 text-sky-400" /> Log Web Dev Task / Topic
              </h3>
              <button
            ...
            <form onSubmit={handleLogSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Task / Question Title *</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Build Custom React Hook for Debounce"
                  value={title}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    <option value="Fullstack">Fullstack REST API</option>
                    <option value="CSS Styling">CSS & Glassmorphism</option>
                    <option value="Database">Database & SQL/JSON</option>
                    <option value="DevOps">DevOps & Deployment</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Topic / Sub-area</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Github / Live Demo URL (Optional)</label>
                <input
                  type="url"
      ...
      {showApproachModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card max-w-xl w-full p-6 space-y-4 border border-purple-500/40 animate-scaleUp">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <FileText className="w-5 h-5 text-purple-400" /> Upload Approach & Concept Notes
              </h3>
              <button
            ...
            <form onSubmit={handleApproachSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Select Web Dev Task *</label>
                <select
                  required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Architecture & Approach Notes *</label>
                <textarea
                  rows="6"
                  required
                  placeholder="Explain how you built it, key architecture decisions, state flow, or API patterns..."
                  value={approachNotes}
      ...
      {showCodeModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card max-w-2xl w-full p-6 space-y-4 border border-indigo-500/40 animate-scaleUp">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <Code2 className="w-5 h-5 text-indigo-400" /> Upload Solution / Component Code
              </h3>
              <button
            ...
            <form onSubmit={handleCodeSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Full Solution / Component Code Snippet *</label>
                <textarea
                  rows="10"
                  required
                  placeholder="// Paste your full HTML, CSS, JavaScript, or React code snippet here..."
                  value={solutionCode}
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
                ...
                <button type="submit" className="btn-accent text-xs py-2 px-5">
                  Save Solution Code
                </button>
              </div>
            </form>
      ...
      {viewingTask && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card max-w-3xl w-full max-h-[85vh] overflow-y-auto p-6 space-y-5 border border-sky-500/40 animate-scaleUp">
            ...
            <div className="space-y-2">
              <h4 className="text-sm font-bold text-purple-300 flex items-center gap-1.5">
                <FileText className="w-4 h-4 text-purple-400" /> Architecture & Approach Notes
              </h4>
              <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800 text-xs text-slate-200 leading-relaxed whitespace-pre-wrap">
                {viewingTask.approach || 'No approach notes uploaded for this task yet.'}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-bold text-sky-300 flex items-center gap-1.5">
                  <Code2 className="w-4 h-4 text-sky-400" /> Full Solution Code
                </h4>
              ...
              {viewingTask.solution_code ? (
                <pre className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs font-mono text-sky-300 overflow-x-auto leading-relaxed shadow-inner max-h-80">
                ...
                <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 text-xs text-slate-400 italic">
                  No solution code snippet attached for this task yet.
                </div>
              )}
            </div>
